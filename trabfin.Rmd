...
```{r}
# Gráfico de Barras para Ciclo Escolar (3 e 4)
plot_ciclo <- ggplot(bd, aes(x = factor(cicloescolar))) +  # Converter para fator temporariamente
  geom_bar(fill = c("#1f78b4", "#33a02c")) +  # Cores diretas para 3 e 4
  scale_x_discrete(labels = c("3" = "3º Ciclo", "4" = "Secundário")) +  # Rótulos claros
  labs(title = "Ciclo Escolar",
       x = "",
       y = "Contagem") +
  theme_minimal()

# Gráfico de Densidade para Idade (P3)
plot_idade <- ggplot(bd, aes(x = P3)) +
  geom_density(fill = "#e31a1c", alpha = 0.5) +
...
N <- round(nrow(bd))
Média <- round(mean(bd$P3, na.rm = TRUE))
Mediana <- round(median(bd$P3, na.rm = TRUE))
DesPad <-round(sd(bd$P3, na.rm = TRUE))
Vari <- round(var(bd$P3, na.rm = TRUE))
Min <- round(min(bd$P3, na.rm = TRUE))
Max <- round(max(bd$P3, na.rm = TRUE))
Q1 <- round(quantile(bd$P3, 0.25, na.rm = TRUE))
Q3 <- round(quantile(bd$P3, 0.75, na.rm = TRUE))
